...
function lockDay() {
  ...
  if (!dayLog.mainTimeHandled) {
    alert("Du m\u00e5 fullf\u00f8re hovedtimef\u00f8ring f\u00f8r du kan godkjenne dagen.");
    return;
  }
...
function skipPreDaySchema(schemaId) {
  ...
  if (isSchemaRequired(schema.type)) {
    alert("Dette skjemaet er p\u00e5krevd av arbeidsgiver og kan ikke hoppes over.");
    return;
  }
...
function skipAllPreDay() {
  ...
  saveCurrentDay();
  renderPreDayContent();  // Re-render to show updated state
}
...
function closeSchemaEdit() {
  ...
  clearUxState();
  document.getElementById("schemaEditOverlay").classList.add("hidden");
}
...
function confirmMainTimeEntry() {
  ...
  if (!draft.lonnskoder || draft.lonnskoder.length === 0) {
    alert("Du m\u00e5 legge til minst \u00e9n l\u00f8nnskode");
    return;
  }
...
function discardMainTimeEntry() {
  ...
  // Only allow discard if ALL time is on tillegg
  if (lockedInfo.totalHours <= 0) {
    alert("Du kan ikke forkaste hovedtimearket n\u00e5r ingen timer er f\u00f8rt p\u00e5 tilleggsoppdrag.");
    return;
  }
  ...
  // Set flag: main time handled (via discard)
  dayLog.mainTimeHandled = true;
...
// --- Keyboard support for edit overlay ---
